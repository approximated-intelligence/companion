...
import de.perigon.companion.track.data.TrackSegmentWithPoints
...
object GpxExporter {
    ...
    fun export(track: TrackWithSegments, output: OutputStream) {
        OutputStreamWriter(output, Charsets.UTF_8).use { w ->
            ...
            w.write("\n")
            w.write("  <trk>\n")
            w.write("    <name>${escapeXml(track.track.name)}</name>\n")
    ...
    private fun writeSegment(w: OutputStreamWriter, seg: TrackSegmentWithPoints) {
        w.write("    <trkseg>\n")
